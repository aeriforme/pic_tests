...
for n,ts in enumerate(steps):
    ...
    #extracting Rho ele
    Rhoec = j.meshes["rho_elec"][io.Mesh_Record_Component.SCALAR]
    Rhoec_data = Rhoec.load_chunk()
    Rhoef = j.meshes["rho_elef"][io.Mesh_Record_Component.SCALAR]
    Rhoef_data = Rhoef.load_chunk()
    Rhoes = j.meshes["rho_eles"][io.Mesh_Record_Component.SCALAR]
    Rhoes_data = Rhoes.load_chunk()

    #extracting Rho contaminants                                                                                                                                                   
    Rhoic = j.meshes["rho_ionc"][io.Mesh_Record_Component.SCALAR]
    Rhoic_data = Rhoic.load_chunk()
    
    #actual loading of data
    series.flush()    

    Rhoe_data = -(Rhoec_data+Rhoes_data+Rhoef_data)
    ...
    lineout_Rhoi_w = Rhoic_data[int(nz*0.5),:]
    lineout_ni_w = lineout_Rhoi_w/(q_e*n_crit)
    ...
    nes = s.Field(0,'Rho_ele_s', units=["um", "fs"], timesteps=ts)
    nef = s.Field(0,'Rho_ele_f', units=["um","fs"], timesteps=ts)
    nec = s.Field(0,'Rho_ele_c', units=["um","fs"], timesteps=ts) 
    nic = s.Field(0,'Rho_ion_c', units=["um","fs"], timesteps=ts)
    
    time_s = Ey.getTimes()
    
    Ey_data = Ey.getData()
    Bz_data = Bz.getData()
    ne_data = nes.getData()+nef.getData()+nec.getData()
    ni_data = nic.getData()
    ...
    lineout_ni_s = ni_data[:,int(np.shape(ni_data)[1]*0.5)]
    ...
    ne = (raw.__dict__["Derived_Number_Density_ele_subs"].data)/n_crit + (raw.__dict__["Derived_Number_Density_ele_foam"].data)/n_crit
    ne = -1*(ne + ((raw.__dict__["Derived_Number_Density_ele_cont"].data)/n_crit))
    ni = (raw.__dict__["Derived_Number_Density_ion_cont"].data)/n_crit
    
    lineout_Ey_e = Ey[:,int(np.shape(Ey)[1]*0.5)]
    lineout_Bz_e = Bz[:,int(np.shape(Bz)[1]*0.5)]
    lineout_ne_e = ne[:,int(np.shape(ne)[1]*0.5)]
    lineout_ni_e = ni[:,int(np.shape(ni)[1]*0.5)]
    
    ##################
    # plot images
    fig, ax = plt.subplots(ncols=2, nrows=2, figsize=(3000./my_dpi, 1000./my_dpi), dpi=my_dpi)

    ax[0,0].plot(gridx_w, lineout_Ez_w, lw=1, label='warpx')  
    ax[0,0].plot(gridx_s, lineout_Ey_s, lw=1, label='smilei')
    ax[0,0].plot(gridx_e, lineout_Ey_e, lw=1, label='epoch')

    ax[0,1].plot(gridx_w, lineout_By_w, lw=1, label='warpx')  
    ax[0,1].plot(gridx_s, lineout_Bz_s, lw=1, label='smilei')
    ax[0,1].plot(gridx_e, lineout_Bz_e, lw=1, label='epoch') 
    
    ax[1,0].plot(gridx_w, lineout_ne_w, lw=1, label='warpx')  
    ax[1,0].plot(gridx_s, lineout_ne_s, lw=1, label='smilei')
    ax[1,0].plot(gridx_e, lineout_ne_e, lw=1, label='epoch')     

    ax[1,1].plot(gridx_w, lineout_ni_w, lw=1, label='warpx')
    ax[1,1].plot(gridx_s, lineout_ni_s, lw=1, label='smilei')
    ax[1,1].plot(gridx_e, lineout_ni_e, lw=1, label='epoch')
    
    ax[0,0].set_xlabel(r'x [$\mu$m]')
    ax[0,0].set_ylabel(r'E$_y$ [V/m]')
    ax[0,0].legend()
    ax[0,0].set_title('transverse E field at mid y')
    
    ax[0,1].set_xlabel(r'x [$\mu$m]')
    ax[0,1].set_ylabel(r'B$_z$ [T]')
    ax[0,1].legend()
    ax[0,1].set_title('transverse B field at mid y')

    ax[1,0].set_xlabel(r'x [$\mu$m]')
    ax[1,0].set_ylabel(r'n$_e$ [n$_{crit}$]')
    ax[1,0].legend()
    ax[1,0].set_title('electron density at mid y')

    ax[1,1].set_xlabel(r'x [$\mu$m]')
    ax[1,1].set_ylabel(r'n$_e$ [n$_{crit}$]')
    ax[1,1].legend()
    ax[1,1].set_title('proton density at mid y')
