...
for d in list_dirs:
    ...
    for t in (count-2, ):
        ...
        x=raw.__dict__["Grid_Particles_ele_sub"].data[0]/micron
        y=raw.__dict__["Grid_Particles_ele_sub"].data[1]/micron
        z=raw.__dict__["Grid_Particles_ele_sub"].data[2]/micron
        px=raw.__dict__["Particles_Px_ele_sub"].data/(m_e*c)
        py=raw.__dict__["Particles_Py_ele_sub"].data/(m_e*c)
        pz=raw.__dict__["Particles_Pz_ele_sub"].data/(m_e*c)
        E=m_e*c**2*(np.sqrt(1.+px**2+py**2+pz**2)-1.)/e*1e-6
        index = (E>=Emin) & (E<=Emax) 
        
        np.savetxt('ELESUB_NOTSOHOT_X_Y_Z_PX_PY_PZ_E_%04d_%s.txt' % (t,d), np.c_[x[index],y[index],z[index],px[index],py[index],pz[index],E[index]],fmt='%.6e')
