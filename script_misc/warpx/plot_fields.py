...
for n,i in enumerate(series.iterations,start=0):
    ...
    Ex_data=Ex.load_chunk()[::2,::2,:]
    Ey_data=Ey.load_chunk()[::2,::2,:]
    #Ez_data=Ez.load_chunk()
    #Bx_data=Bx.load_chunk()
    #By_data=By.load_chunk()
    Bz_data=Bz.load_chunk()[::2,::2,:]
    ...
    h = int(0.5*np.shape(Ey_data)[0])
    my_dpi = 300
    fig, axs = plt.subplots(ncols=6, nrows=1)
    fig.set_size_inches(6000./my_dpi, 1000./my_dpi)
    fig.set_dpi(my_dpi)
    axs[0].imshow(Ex_data[h,:,:],cmap='bwr',norm=Normalize(Ex_data.min(),Ex_data.max()))
    axs[0].set_title(r'E$_x$')
    axs[1].imshow(Ey_data[h,:,:],cmap='bwr',norm=Normalize(Ey_data.min(),Ey_data.max()))
    axs[1].set_title(r'E$_y$')

    
    #axs[2].imshow(rho_ele_foam_data[h,:,:],cmap='viridis')
    #axs[3].imshow(rho_ele_subs_data[h,:,:],cmap='viridis')

    #axs[2].set_title(r'E$_z$')
    #axs[3].imshow(Bx_data[:,:,h], cmap='bwr',norm=Normalize(Bx_data.min(),Bx_data.max()))
    #axs[3].set_title(r'B$_x$')
    #axs[4].imshow(By_data[:,:,h],cmap='bwr',norm=Normalize(By_data.min(),By_data.max()))
    #axs[4].set_title(r'B$_y$')
    axs[5].imshow(Bz_data[h,:,:],cmap='bwr',norm=Normalize(Bz_data.min(),Bz_data.max()))
    axs[5].set_title(r'B$_z$')

    fig.suptitle('t = {0}'.format(j.time))
    
    img_file_name ='./EB_%06d.png' % i 
